...
def model_predict_onnx(model, image_data, class_names):
    input_tensors = []
...
def model_predict_mnn(interpreter, session, image_data, class_names):
    # assume only 1 input tensor for image
...
def eval_PCK(model, model_format, eval_dataset, class_names, model_input_shape, score_threshold, normalize, conf_threshold, save_result=False, skeleton_lines=None):
    ...
    for image_data, gt_heatmap, metainfo in eval_dataset:
        ...
        if model_format == 'TFLITE':
            heatmap = model_predict_tflite(model, image_data)
        # support of MNN model
        elif model_format == 'MNN':
            heatmap = model_predict_mnn(model, session, image_data, class_names)
        # support of TF 1.x frozen pb model
        elif model_format == 'PB':
            heatmap = model_predict_pb(model, image_data)
        # support of ONNX model
        elif model_format == 'ONNX':
            heatmap = model_predict_onnx(model, image_data, class_names)
        # normal keras h5 model
        elif model_format == 'H5':
            heatmap = model_predict_keras(model, image_data)
        else:
